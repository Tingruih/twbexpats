...
**第二層（statcast 管線另外合併進同一個 dict）**：`statcast` / `expected` 由 `sync/statcast.py::_attach_statcast()` 只寫入 `sport_level` 相符的那一列；其餘進階欄位由 `sync/advanced.py::sync_season_advanced()` 掃過每一列寫入，不看該年有沒有逐球資料：
...
| `saber` | dict | 僅 MLB。`api/stats.py::get_player_sabermetrics()` 的**整季合計** split `stat`（含 `fip`/`xfip`/`war`/`wRcPlus`）；轉隊球員不取各隊 split（見 `sync/advanced.py::_season_total_saber()`），該年每一隊的 MLB 列都存同一份。當季每次重抓；過去球季抓過一次後登記在 `season_fetches`，之後 `fip`/`war`/`wrc_plus` 等欄位改由這份存下的 `saber` 重算 |
| `expected` | dict | 僅 MLB。`api/stats.py::get_player_expected_stats()` 的原始回傳（`avg`/`slg`/`woba`/`wobaCon`）。抓取規則同 `saber`（`season_fetches`）；沒抓的年份保留既有值 |
...
| `war` | float | 僅 MLB，直接取自 `saber.war`；整季值，該年每一隊的 MLB 列相同 |
| `wrc_plus` | int | 僅 MLB。`saber.wRcPlus` 四捨五入；整季值，該年每一隊的 MLB 列相同（render 以 `_first_not_none` 取值） |
...
**誰寫**：`sync/players.py::_write_player_to_db()`（基礎欄位）、`sync/statcast.py::_attach_statcast()`（`statcast`/`expected`）、`sync/advanced.py::sync_season_advanced()`（`saber`/`fip`/`xfip`/`war`/`wrc_plus`/`lg_era`/`xwpct`）。
...
| `pitches_json` | TEXT | DEFAULT `'[]'` | 逐球資料，見下。`'[]'` = 尚未抓取或該球員這場沒有逐球資料；是否已抓看 `pbp_version`，不看這欄 |
| `events_json` | TEXT | DEFAULT `'[]'` | 逐非投球事件，見下 |
| `sport_level` | TEXT | DEFAULT `''` | tier key（`MLB`/`AAA`/`AA`/`A+`/`A`/`A-`/`ROA`/`ROK`/`WIN`，見 `levels.py`），寫入一律經 `levels.sport_to_tier_key()`；**不存舊制名稱**（`A(Adv)` 等由 `level_display(key, year)` 依年份產生）；**注意：這欄不在原始 `CREATE TABLE` 裡，見 §8** |
...
- `sync/statcast.py`：`pitches_json`/`events_json`/`sport_level`/`pbp_version`（playByPlay 階段，`_fetch_and_extract_game()` 抓、`_write_pitch_logs()` 寫；未完賽的比賽寫入部分資料但不更新 `pbp_version`）
...
改為逐 (球員, 比賽) 判斷（現為 `game_logs.pbp_version`，見 `sync/statcast.py::_games_to_fetch()`）後就不再讀取。
...
| 欄位 | 型別 | 說明 |
|---|---|---|
| `source` | TEXT | NOT NULL，資料來源：`sabermetrics` / `expected` / `fip_constants`（常數定義在 `db/season_fetches.py`） |
| `subject` | TEXT | NOT NULL，`sabermetrics`/`expected` 為球員 mlb_id（字串），`fip_constants` 為 tier key |
...
也會登記，抓取失敗（`FetchError`）不登記、下次重試。`--full-history`（`sabermetrics`/`expected`）與
...
- **`game_logs.sport_level`、`game_logs.pbp_version` 至今仍然只存在於
...
- **`game_logs.hit_coord_checked` → `pbp_version`（2026-09）**：`hit_coord_checked`
  ...
  之後 `init_db()` 的 `_add_column` / `_drop_column` 會因欄位已存在/不存在而略過。
